...
import 'package:pradeep_the_developer/fullApps/cloudStorage/components/CSFileAndFolderEditingComponents.dart';
...
class CSDisplayDataInListViewComponents extends StatefulWidget {
  ...
  @override
  CSDisplayDataInListViewComponentsState createState() =>
      CSDisplayDataInListViewComponentsState();
}

class CSDisplayDataInListViewComponentsState
    extends State<CSDisplayDataInListViewComponents> {
  @override
  void initState() {
    super.initState();
    init();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return ListTile(
          ...
          onTap: () {
            if (widget.listOfData![index].isFolder) {
              widget.isSelect!
                  ...
                  : widget.isCopyOrMove!
                      ? toasty(context,
                          "Copied ${widget.listOfData![index].fileName}")
          ...
          leading: commonCachedNetworkImage(widget.listOfData![index].fileUrl,
              height: 30, width: 30),
          trailing: Row(
                          ...
                          onChanged: (val) {
                            setState(() {
                              ...
                              widget.onChange!.call(
                                  widget.selectedItem, widget.isSelectAll);
                            });
                          },
                        ),
                      ],
          ...
          subtitle: widget.listOfData![index].fileUrl != CSFolderIcon
              ? Text("size and time display here")
              : null,
        );
      },
  ...
  void selectFileAndFolders(int index) {
    setState(
      () {
        widget.isSelectAll = false;
        widget.listOfData![index].isFileSelect =
            !widget.listOfData![index].isFileSelect!;
        widget.listOfData![index].isFileSelect!
            ? widget.selectedItem = widget.selectedItem! + 1
            : widget.selectedItem = widget.selectedItem! - 1;
