...
import 'package:pradeep_the_developer/integrations/models/TestCardModel.dart';
...
class CheckOutComponentState extends State<CheckOutComponent> {
  ...
  List<String> cardList = [
    'images/integrations/img/card1.jpg',
    'images/integrations/img/card2.png'
  ];
  ...
  Future<void> init() async {
    ...
    testCard = [
      TestCard(
          cardTitle: 'Card Network : ',
          cardValue: widget.cardNetwork.validate()),
      TestCard(
          cardTitle: 'Card Number : ', cardValue: widget.cartNumber.validate()),
      TestCard(cardTitle: 'CVV : ', cardValue: widget.cardCVV.validate()),
      TestCard(
          cardTitle: 'Expiry Date : ', cardValue: widget.expiryDate.validate()),
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Image.asset('images/integrations/img/credit_card.png',
                width: context.width(), height: 200, fit: BoxFit.fill)
            ...
            children: [
              if (widget.cartNumber.validate().isNotEmpty)
                Column(
                  ...
                  children: [
                    8.height,
                    Text('Test Card for Domestic Payment',
                      ...
                      children: testCard.map((e) {
                        return e.cardValue.validate().isNotEmpty
                            ? Row(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(e.cardTitle.validate(),
                                          style: primaryTextStyle(size: 14))
                                      .expand(),
                                  Text(e.cardValue.validate(),
                                          style: boldTextStyle(),
              ...
              if (widget.warning.validate().isNotEmpty)
                Text('Note : ' + widget.warning.validate(),
                    style: primaryTextStyle(color: Colors.red, size: 12))
            ],
          ),
        ),
        16.height,
